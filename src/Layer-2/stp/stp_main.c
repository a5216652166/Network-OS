...
static void stp_send_bpdu(struct stp_port_entry *p,  char *p_out,
			 int length)
{
	uint8_t  smac[6];


	llc_pdu_header_init (p_out , LLC_PDU_TYPE_U, LLC_SAP_BSPAN, LLC_SAP_BSPAN, 
			     LLC_1_PDU_CMD_UI);
...
void stp_send_config_bpdu(struct stp_port_entry *p, STP_BPDU_T *bpdu)
{
	...
	buf = calloc (1, sizeof (STP_BPDU_T));

	if (!buf)
		return;

	buf += sizeof (LLCHDR) + sizeof (MACHDR);
	...
	stp_send_bpdu(p, buf, sizeof (STP_HDR_T) + sizeof (LLCHDR));
}
...
void stp_send_tcn_bpdu(struct stp_port_entry *p)
{
	...
	stp_send_bpdu(p, buf, 4 + sizeof (LLCHDR));
}
